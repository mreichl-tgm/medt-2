function datePickKalender(date, datum, monat, jahr)
{
	...
	document.write('<script type="text/javascript" src="datePickKalender.js"></script>');
	document.write('<script type="text/javascript" src="kalender_next.js"></script>'); 
	document.write('<script type="text/javascript" src="kalender_prev.js"></script>');
	...
	for(j=1; j<32; j++)
	{
		...
		if(monat == date.getMonth() && j == datum)
		{
			...
			document.write('<td id=tag><b><a href="#" onClick="SetDate(+j+.+monat+.+jahr+);">', j, '</a></b></td>');
		} else {
			document.write('<td><a href="#" id="picker" onClick="SetDate(+j+.+monat+.+jahr+);">',j,'</a></td>');
		}
